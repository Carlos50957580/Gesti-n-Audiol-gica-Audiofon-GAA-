    ...
    <table class="table inv-table mb-0">
        <thead>
            <tr>
                <th># Factura</th>
                <th>Paciente</th>
                {{-- Admin y admin2 pueden ver sucursal --}}
                @if(auth()->user()->role->name === 'admin' || $isAdmin2)
                <th>Sucursal</th>
                @endif
                <th>Seguro</th>
                ...
                @endif
                <th>Desc. Seguro</th>
                {{-- Ocultar Total solo para admin2 --}}
                @if(!$isAdmin2)
                <th>Total</th>
                @endif
                <th>Estado</th>
        ...
        <tbody>
            ...
            @empty
            <tr>
                {{-- Calcular colspan dinámicamente según el rol --}}
                @php
                    $colspan = 7; // Base: #Factura, Paciente, Seguro, Desc.Seguro, Estado, Fecha, Acciones
                    if(auth()->user()->role->name === 'admin' || $isAdmin2) $colspan++; // Sucursal
                    if(!$isAdmin2) $colspan += 2; // Subtotal y Total
                @endphp
                <td colspan="{{ $colspan }}">
