...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">Editar Categoría: {{ $serviceCategory->name }}</h4>
                ...
                <form action="{{ route('service-categories.update', $serviceCategory) }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="icon" class="form-control @error('icon') is-invalid @enderror" 
                                       value="{{ old('icon', $serviceCategory->icon ?? 'ri-folder-line') }}" placeholder="ri-folder-line">
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <span class="input-group-text" id="colorPreview" style="background-color: {{ old('color', $serviceCategory->color ?? '#405189') }}; width: 40px;"></span>
                                <input type="color" name="color" class="form-control form-control-color" 
                                       value="{{ old('color', $serviceCategory->color ?? '#405189') }}" style="padding: 3px; max-width: 60px;">
                                <input type="text" id="colorText" class="form-control" 
                                       value="{{ old('color', $serviceCategory->color ?? '#405189') }}" placeholder="#405189">
                            </div>
                            @error('color')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-check form-switch mt-2">
                                ...
                                <input type="checkbox" name="requires_clinical_record" class="form-check-input" id="requiresClinicalRecord" 
                                       value="1" {{ old('requires_clinical_record', $serviceCategory->requires_clinical_record) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-check form-switch mt-2">
                                ...
                                <input type="checkbox" name="is_active" class="form-check-input" id="isActive" 
                                       value="1" {{ old('is_active', $serviceCategory->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="flex-shrink-0 me-3">
                                <div class="avatar-sm">
                                    <span class="avatar-title rounded-circle fs-16" id="previewBadge"
                                          style="background-color: {{ old('color', $serviceCategory->color ?? '#405189') }}20; color: {{ old('color', $serviceCategory->color ?? '#405189') }}">
                                        <i class="{{ old('icon', $serviceCategory->icon ?? 'ri-folder-line') }}" id="previewIcon"></i>
                                    </span>
                                </div>
                            </div>
                            <div class="flex-grow-1 text-start">
                                <h5 class="mb-1" id="previewName">{{ old('name', $serviceCategory->name) }}</h5>
                                <p class="text-muted mb-0" id="previewDescription">{{ old('description', $serviceCategory->description ?? 'Sin descripción') }}</p>
                            </div>
                        </div>
                        <div class="mt-2 pt-2 border-top">
                            <div class="d-flex justify-content-between">
                                <span class="text-muted">Historia Clínica:</span>
                                <span class="badge {{ old('requires_clinical_record', $serviceCategory->requires_clinical_record) ? 'bg-success' : 'bg-secondary' }}" id="previewHc">
                                    {{ old('requires_clinical_record', $serviceCategory->requires_clinical_record) ? 'Sí' : 'No' }}
                                </span>
                            </div>
                            <div class="d-flex justify-content-between mt-1">
                                <span class="text-muted">Estado:</span>
                                <span class="badge {{ old('is_active', $serviceCategory->is_active) ? 'bg-success' : 'bg-danger' }}" id="previewStatus">
                                    {{ old('is_active', $serviceCategory->is_active) ? 'Activo' : 'Inactivo' }}
                                </span>
                            </div>
                        </div>
                    </div>
