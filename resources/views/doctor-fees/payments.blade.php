...
<div class="container-fluid pt-3">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-0">Pagos a Médicos</h4>
    ...
    <div class="card shadow-sm" style="border-radius:.75rem;border:none;">
        <div class="card-header d-flex align-items-center gap-3 flex-wrap py-3" style="border-bottom:1px solid #f0f2f7;">
            ...
            <select id="doctor-filter" class="form-select" style="width:180px;">
            ...
            <div class="table-responsive">
                <table class="table payments-table mb-0">
                    ...
                    <tbody id="payments-tbody">
                        ...
                        <tr class="anim-row"
                            data-id="{{ $payment->id }}"
                            data-doctor="{{ strtolower($payment->doctor->name) }}"
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius:.75rem;overflow:hidden;">
            <div class="modal-header mh-success py-3">
                <h5 class="modal-title d-flex align-items-center gap-2">
                    <i class="ri-bank-card-line fs-18"></i> Registrar Pago a Médico
                </h5>
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <select id="payment-doctor" class="form-select" required>
                ...
                <div id="fees-container">
                    ...
                    <div class="text-center py-3" id="no-fees-message">
                        ...
                        <p class="text-muted mt-2 mb-0">Seleccione un Médico para ver los honorarios pendientes</p>
                    </div>
...
const URL_BASE = "{{ route('doctor-fees.payments') }}";
const URL_PENDING_FEES_BASE = "{{ url('doctor-fees/payments/pending') }}";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (doctorSelect) {
        doctorSelect.addEventListener('change', loadPendingFees);
    }
    ...
    if (doctorFilter) doctorFilter.addEventListener('change', filterTable);
...
async function loadPendingFees() {
    const doctorId = document.getElementById('payment-doctor')?.value;
    ...
    try {
        const url = `${URL_PENDING_FEES_BASE}/${doctorId}`;
        ...
        if (!fees || fees.length === 0) {
            ...
            if (noFeesMsg) {
                ...
                noFeesMsg.innerHTML = `
                    ...
                    <p class="text-success mt-2 mb-0">No hay honorarios pendientes para este Médico</p>
                `;
            }
            return;
        }
...
function renderPendingFeesTable(fees) {
    ...
    fees.forEach(fee => {
        ...
        row.innerHTML = `
            ...
            <td>${fee.invoice?.invoice_number ?? 'FAC-'+String(fee.invoice_id).padStart(6, '0')}</td>
...
async function savePayment() {
    const doctorId = document.getElementById('payment-doctor')?.value;
    if (!doctorId) {
        const errElement = document.getElementById('err-doctor_id');
        if (errElement) {
            errElement.textContent = 'Seleccione un Médico';
            document.getElementById('payment-doctor')?.classList.add('is-invalid');
        }
        return;
    }
                            ...
                            <div class="card-body">
                                ...
                                <div class="detail-row p-0 border-0">
                                    ...
                                    <div>
                                        ...
                                        <div class="detail-val">${payment.doctor?.name || 'N/A'}</div>
                                    </div>
                                </div>
                                <div class="detail-row p-0 border-0">
                                ...
                                <div class="detail-row p-0 border-0">
                                    ...
                                    <div>
                                        ...
                                        <div class="detail-val">${payment.reference_number}</div>
                                    </div>
                                </div>
                                ` : ''}
                                ${payment.notes ? `
                                <div class="detail-row p-0 border-0">
                                    ...
                                    <div>
                                        ...
                                        <div class="detail-val">${payment.notes}</div>
                                    </div>
                                </div>
                                ` : ''}
                            </div>
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .detail-row { display:flex; gap:.75rem; align-items:flex-start; padding:.5rem 0; border-bottom:1px solid #f3f5f9; }
                .detail-row:last-child { border-bottom:none; }
                .detail-icon { width:32px; height:32px; border-radius:.4rem; display:flex; align-items:center; justify-content:center; flex-shrink:0; font-size:.9rem; }
                .detail-lbl { font-size:.7rem; font-weight:700; letter-spacing:.06em; text-transform:uppercase; color:#8098bb; }
                .detail-val { font-size:.85rem; font-weight:500; color:#344563; }
                .method-badge { display:inline-flex; align-items:center; gap:.4rem; padding:.2rem .6rem; border-radius:2rem; font-size:.7rem; font-weight:600; }
                .method-cash { background:#d1fae5; color:#065f46; }
                .method-bank_transfer { background:#e0e7ff; color:#3730a3; }
                .method-check { background:#fef3c7; color:#92400e; }
                .method-other { background:#e5e7eb; color:#374151; }
                .section-label { font-size:.7rem; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:#8098bb; border-bottom:1px solid #e9ecef; padding-bottom:.4rem; margin-bottom:.9rem; }
