...
<div class="container-fluid pt-3">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-0">Honorarios Médicos</h4>
        ...
        <div class="col-6 col-xl-3">
            <div class="card stat-card shadow-sm h-100">
                <div class="card-body d-flex align-items-center gap-3 py-3">
                    ...
                    <div>
                        <div class="text-muted" style="font-size:.72rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;">Médicos</div>
                        <div class="fw-bold fs-4 lh-1 mt-1">{{ $stats['doctors_count'] }}</div>
                    </div>
    ...
    <div class="card shadow-sm" style="border-radius:.75rem;border:none;">
        <div class="card-header d-flex align-items-center gap-3 flex-wrap py-3" style="border-bottom:1px solid #f0f2f7;">
            ...
            <select id="doctor-filter" class="form-select filter-select" style="width:180px;">
            ...
            <div class="table-responsive">
                <table class="table fee-table mb-0">
                    ...
                    <tbody id="fees-tbody">
                        ...
                        <tr class="anim-row"
                            data-id="{{ $fee->id }}"
                            data-doctor="{{ strtolower($fee->doctor->name) }}"
                            data-status="{{ $fee->status }}">
                            <td>#{{ $fee->id }}</td>
                            <td>
                                <span class="fw-semibold">
                                    {{ $fee->invoice->invoice_number ?? 'FAC-'.str_pad($fee->invoice_id, 6, '0', STR_PAD_LEFT) }}
                                </span>
                            </td>
                            <td>{{ $fee->doctor->name }}</td>
            ...
            <div class="modal-body p-4" style="max-height:70vh; overflow-y:auto;">
                ...
                <div class="detail-row">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div class="detail-val" id="show-doctor">—</div>
                    </div>
                </div>
...
const URL_BASE = "{{ route('doctor-fees.index') }}".replace(/\/+$/, '');
...
document.getElementById('doctor-filter').addEventListener('change', filterTable);
...
function filterTable() {
    ...
    rows.forEach(tr => {
        ...
        if (doctorId && !tr.cells[2]?.textContent.includes(document.querySelector('#doctor-filter option:checked')?.text)) show = false;
...
async function openShowModal(id) {
    ...
    try {
        ...
        // Información del Honorario
        document.getElementById('show-invoice').innerHTML = data.invoice?.invoice_number || 'FAC-'+String(data.invoice_id).padStart(6, '0');
        document.getElementById('show-doctor').textContent = data.doctor?.name || '—';
