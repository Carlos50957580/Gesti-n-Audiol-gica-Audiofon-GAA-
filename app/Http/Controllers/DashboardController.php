...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Solo admin ve todas las sucursales; recepcionista y médico ven su sucursal
        $invQ = Invoice::query();
        $apptQ = Appointment::query();
        $patQ = Patient::query();

        if (!$isAdmin) {
            $invQ->where('branch_id', $branchId);
            $apptQ->where('branch_id', $branchId);
            $patQ->where('branch_id', $branchId);
        }
        ...
        // ── Top médicos este mes (usuarios con is_doctor = 1) ──────────────────
        ...
        // ── Para médico: sus citas de hoy ─────────────────────────────────────
        $myTodayAppointments = [];
        if ($isDoctor) {
            $myTodayAppointments = Appointment::where('doctor_id', $user->id)
                ->whereDate('appointment_date', $today)
                ->with(['patient', 'branch'])
                ->orderBy('appointment_time')
                ->limit(5)->get();
